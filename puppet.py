...
def main():
    ...
    env = Environment (
        ...
        extensions=[jinjaExampleExt.FragmentCacheExtension, puppetIfExtension.puppetIfExtension, puppetHashExtension.puppetHashExtension],
	    block_start_string="<%",
	    block_end_string="%>",
    ...
    lexed = env.lex(template_str)
    
    for line in lexed:
        logging.info(f'lexed line: \'{line}\'')

    combined = " ".join(lexed)
    logging.info(f"lexed combined: {combined}")

    # preprocessed = env.preprocess(template_str)
    # logging.info(f'the preprocessed template \n\'{preprocessed}\'')
